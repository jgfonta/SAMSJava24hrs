...
{       
	...
	void startPlaying() 
	{ 
		if (numbersUnique(gui.numbers, gui.numbers.length)) {
	...
	boolean numbersUnique( JTextField[] userNums, int count) 
	{
		for (int i = 0; i < count; i++) 
		{ 
			for (int j = i+1; j < count; j++)
			{
				if (Integer.parseInt(userNums[i].getText()) == Integer.parseInt(userNums[j].getText())) 
				{	 
					userNums[i].setBackground(Color.RED);
					return false; 
				}
			}
			userNums[i].setBackground(Color.WHITE);
		} 		
		return true;
	}
